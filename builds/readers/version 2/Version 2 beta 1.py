...
encsign1 = "b"
encsign2 = " "
encsign3 = "  <!--Auto Stub-->c"

for s in range(len(splitSteg)):
     find = splitSteg[s].find(encsign3)
     print(find)
     if (splitSteg[s].endswith(encsign) or splitSteg[s].endswith(encsign1) or find != -1): #reduce to how many steg factors you have
         #start reading

         steg_char_num_a = len(splitSteg[s])-len(splitSteg[s].rstrip(steg_char_value1)) #length of each line - the length of the non whitespace
         ...
         elif (steg_char_num_c != 0):
             plaintext.append(kb4_uppercase[steg_char_num_c])
             print(steg_char_num_c)
