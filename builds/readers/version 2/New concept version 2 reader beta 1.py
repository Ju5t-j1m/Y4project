...
for s in range(len(splitSteg)):
     ...
     find4 = splitSteg[s].find(encsign4)
         #start reading
     ...
     if (splitSteg[s].startswith(encsign) and not splitSteg[s].startswith(encsign1) and not splitSteg[s].startswith(encsign2)) and not splitSteg[s].find(encsign3) != -1:
         ...
         print("third third")
          ...
          steg_char_num_a = len(splitSteg[s]) - len(splitSteg[s].lstrip(steg_char_value1))
          if steg_char_num_a >= 2:
              plaintext.append(rebuilt_kb[steg_char_num_a + 27])
              print("number")

          elif splitSteg[s].startswith("\t"):
              tab_cleaned_line = splitSteg[s].replace("\t", '')
              print(tab_cleaned_line)
              steg_char_num_a = len(splitSteg[s]) - len(tab_cleaned_line.lstrip(steg_char_value1))
              print(steg_char_num_a)
              plaintext.append(rebuilt_kb[steg_char_num_a + 38])
          else:
              print("space")
              plaintext.append(' ')
